        ...
        public static Drawing CreateDrawing(this EFontAwesomeIcon icon, Brush primary, Brush secondary = null, bool? swapOpacity = null, double? primaryOpacity = null, double? secondaryOpacity = null, double emSize = 100)
        {
            ...
            using (var drawingContext = visual.RenderOpen())
            {
                ...
                if (icon.IsDuotone())
                {
                    ...
                    var primaryFormattedText = CreateFormattedText($"{info.Item1}", typeface, primaryClone, emSize);
                    var secondaryFormattedText = CreateFormattedText($"{info.Item1}{info.Item1}", typeface, secondaryClone, emSize);

                    drawingContext.DrawText(primaryFormattedText, new Point(0, 0));
                    drawingContext.DrawText(secondaryFormattedText, new Point(0, 0));
                }
                else
                {
